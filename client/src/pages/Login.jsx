...
export default function Login() {
  ...
  const go = async () => {
    ...
    try { const d = await api.login(username, password); setUser(d.user); nav(d.user.role === 'vessel' ? '/vessel' : '/admin'); }
    catch (e) { setError(e.message); } finally { setLoading(false); }
  };
  const inp = "w-full px-4 py-2.5 rounded-lg bg-slate-800/50 border border-white/10 text-slate-200 text-sm focus:outline-none focus:border-cyan-600";
  return (
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl p-6 border border-white/5" style={{ background: 'rgba(15,23,42,0.7)' }}>
          ...
          <div className="space-y-4">
            ...
            <button onClick={go} disabled={loading} className="w-full py-2.5 rounded-lg text-sm font-semibold text-white disabled:opacity-50" style={{ background: 'linear-gradient(135deg,#0F766E,#0E7490)' }}>{loading?'Signing in...':'Sign In'}</button>
          </div>
        </div>
      </div>
    </div>
