...
export default function VesselDashboard({ vesselIdProp, hideIndents = false }) {
  ...
  const [form, setForm] = useState({indent_number:'',title:'',cost_group:'',sub_category:'',cost_element_code:'',source:'HO',status:'Estimate',cost_usd:0,cost_local:0,cost_marked_up:0});
  ...
  const filtered=srcIndents.filter(i=>{
    ...
    if(searchQ){const q=searchQ.toLowerCase();if(!((i.title||'').toLowerCase().includes(q)||(i.indent_number||'').toLowerCase().includes(q)||(i.sub_category||'').toLowerCase().includes(q)||(i.cost_group||'').toLowerCase().includes(q)))return false;}
    return true;
  });
  ...
  const submitIndent=async()=>{try{await api.createIndent(vesselId,form);setShowForm(false);setForm({indent_number:'',title:'',cost_group:'',sub_category:'',cost_element_code:'',source:indentSrc,status:'Estimate',cost_usd:0,cost_local:0,cost_marked_up:0});loadAll();}catch(e){alert(e.message);}};
  ...
  const handleBudgetUpload=async(e)=>{
    ...
    try{const r=await api.uploadBudget(vesselId,file,budgetYear,replace);alert(`Imported ${r.imported} budget lines. Total: $${r.total_budget.toLocaleString()}`);loadAll();}
  ...
  const loadLatestBPR=async()=>{
    setBprLoading(true);
    try{
      const data=await api.getLatestBPR(vesselId,budgetYear);
      if(data){setBprData(data);setBprMeta({uploaded_by:data.uploaded_by,uploaded_at:data.uploaded_at,filename:data.filename});}
      else{setBprData(null);setBprMeta(null);}
    }catch(e){console.error(e);}
    finally{setBprLoading(false);}
  };
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {tab==='categories'&&!drillSub&&<div className="space-y-6">
        ...
        {detail&&detail.subs.length>0&&<div className="rounded-xl p-5 border border-cyan-800/30" style={{background:'rgba(14,116,144,0.05)'}}>
          <h3 className="text-sm font-semibold text-cyan-300 mb-4">{detail.category} — click a sub-category to see its indents</h3>
          <table className="w-full text-xs"><thead><tr className="border-b border-white/10">
            ...
            const sp=s.annual>0?s.actual/s.annual:0;
            const mT=s.annual*(currentMonth/12);
            const mP=mT>0?s.actual/mT:0;
            const mC=mP>1?'#F87171':mP>0.8?'#FBBF24':'#34D399';
            const tC=sp>1?'#F87171':sp>0.5?'#FBBF24':'#34D399';
      ...
      {tab==='categories'&&drillSub&&<div className="space-y-4">
        <div className="flex items-center gap-3">
          <button onClick={()=>setDrillSub(null)} className="px-3 py-1.5 rounded-lg text-xs text-cyan-300 bg-cyan-900/20 border border-cyan-800/30">← Back to categories</button>
      ...
      {tab==='indents'&&!hideIndents&&<div className="space-y-5">
        <div className="flex items-center justify-between">
          ...
          {canEdit&&<button onClick={()=>{setForm({...form,source:indentSrc==='CF'?'HO':indentSrc});setShowForm(!showForm);}} className="px-4 py-2 rounded-lg text-sm font-semibold text-white" style={{background:'linear-gradient(135deg,#0F766E,#0E7490)'}}>+ New Indent</button>}
        </div>
        ...
        {showForm&&canEdit&&<div className="rounded-xl p-4 border border-cyan-800/30" style={{background:'rgba(14,116,144,0.05)'}}>
          <h4 className="text-sm font-semibold text-cyan-300 mb-3">New Indent</h4>
          ...
          <div className="mt-3"><CostElementPicker value={{cost_group:form.cost_group,sub_category:form.sub_category}} onChange={v=>setForm({...form,...v})}/></div>
          <div className="grid grid-cols-4 gap-3 mt-3">
            ...
            <div><label className="block text-xs text-slate-400 mb-1">Cost USD</label><input type="number" value={form.cost_usd} onChange={e=>setForm({...form,cost_usd:parseFloat(e.target.value)||0})} className={inp}/></div>
            {form.source==='Outport'&&<div><label className="block text-xs text-slate-400 mb-1">Cost NGN</label><input type="number" value={form.cost_local} onChange={e=>setForm({...form,cost_local:parseFloat(e.target.value)||0})} className={inp}/></div>}
          </div>
          <div className="flex gap-3 mt-3"><button onClick={submitIndent} className="px-4 py-2 rounded-lg text-sm font-semibold text-white" style={{background:'#0F766E'}}>Submit</button><button onClick={()=>setShowForm(false)} className="px-4 py-2 text-sm text-slate-400">Cancel</button></div>
        </div>}
        ...
        <div className="rounded-xl border border-white/5 overflow-hidden" style={{background:'rgba(15,23,42,0.6)'}}>
          <table className="w-full text-sm"><thead><tr style={{borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
            ...
            <tr key={i} className="hover:bg-white/[0.03] cursor-pointer transition-colors" style={{borderBottom:'1px solid rgba(255,255,255,0.03)'}} onClick={()=>setSelectedIndent(ind)}>
              ...
              <td className="px-4 py-3 text-xs text-slate-400">{ind.sub_category}</td>
              <td className="px-4 py-3"><StatusBadge status={ind.status}/></td>
              {indentSrc==='Outport'&&<td className="px-4 py-3 text-right text-xs font-mono text-slate-400">{Number(ind.cost_local||0).toLocaleString()}</td>}
              <td className="px-4 py-3 text-right font-mono text-xs font-semibold">{fmt(Number(ind.cost_usd))}</td>
            </tr>
          ))}</tbody></table>
          <div className="px-4 py-3 text-xs text-right text-slate-600" style={{borderTop:'1px solid rgba(255,255,255,0.04)'}}>{filtered.length} of {srcIndents.length} · Total: {fmt(filtered.reduce((s,i)=>s+Number(i.cost_usd),0))}</div>
        </div>
      </div>}
      ...
      {tab==='bpr'&&<div className="space-y-5">
        <div className="rounded-xl p-5 border border-white/5" style={{background:'rgba(15,23,42,0.6)'}}>
          <div className="flex items-center justify-between">
            <div>
              ...
              {bprMeta && <p className="text-xs text-slate-500 mt-1">
                Uploaded by {bprMeta.uploaded_by || 'Unknown'} · {bprMeta.filename || ''} · {bprMeta.uploaded_at ? new Date(bprMeta.uploaded_at).toLocaleDateString() : ''}
              </p>}
              {!bprMeta && !bprLoading && <p className="text-xs text-slate-500 mt-1">{canEditBudget ? 'Upload a monthly BPR Excel to compare against dashboard data' : 'No BPR data available yet. Contact superintendent to upload.'}</p>}
            </div>
            {canEditBudget && (
              <label className="px-4 py-2 rounded-lg text-sm font-semibold text-white cursor-pointer transition-all hover:opacity-90" style={{background:'linear-gradient(135deg,#0F766E,#0E7490)'}}>
                {bprLoading ? 'Processing...' : '📁 Upload BPR Excel'}
                <input type="file" accept=".xlsx,.xls,.csv" onChange={handleBPRUpload} className="hidden" disabled={bprLoading} />
              </label>
            )}
            {!canEditBudget && !bprData && !bprLoading && (
              <button onClick={loadLatestBPR} className="px-4 py-2 rounded-lg text-sm text-cyan-300 bg-cyan-900/20 border border-cyan-800/30">Refresh</button>
            )}
          </div>
        ...
        {bprData && <>
          <div className="flex items-center gap-3 flex-wrap">
            <input value={bprSearch} onChange={e=>setBprSearch(e.target.value)} placeholder="Search by name, code, cost group..." className={`${inp} max-w-xs`}/>
            ...
            {canEditBudget && bprData.comparison.filter(c=>c.status==='unmatched').length > 0 && (
              <button onClick={async()=>{
                ...
                if(!confirm(`Import ${unmatched.length} unmatched BPR items to dashboard as budget lines?`)) return;
                try {
                  await api.importBPRItems(vesselId, unmatched.map(c=>({code:c.code, name:c.sub_category, approved_budget:c.bpr_budget, cost_group:c.cost_group})), budgetYear);
                  alert(`${unmatched.length} items imported successfully. Re-upload BPR to see updated comparison.`);
                  loadAll();
                } catch(e){alert(e.message);}
              }} className="px-3 py-1.5 rounded-lg text-xs font-semibold text-emerald-300 bg-emerald-900/20 border border-emerald-800/30 hover:bg-emerald-900/40 transition-colors ml-auto">
                Import All Unmatched ({bprData.comparison.filter(c=>c.status==='unmatched').length})
              </button>
            )}
          </div>
          ...
          <div className="rounded-xl border border-white/5 overflow-x-auto" style={{background:'rgba(15,23,42,0.6)'}}>
            <table className="w-full text-xs"><thead><tr className="border-b border-white/6">
              ...
              bprData.comparison
                .filter(c => bprFilter === 'All' || c.status === bprFilter)
                .filter(c => !bprSearch || c.sub_category.toLowerCase().includes(bprSearch.toLowerCase()) || (c.code||'').includes(bprSearch) || (c.cost_group||'').toLowerCase().includes(bprSearch.toLowerCase()))
                .map((c, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                      ...
                      <td className="px-3 py-2 text-slate-200 max-w-[200px] truncate">{c.sub_category}</td>
                      <td className="px-3 py-2 text-slate-500">{c.cost_group}</td>
                      <td className="px-3 py-2 text-right font-mono text-cyan-300">{fmt(c.bpr_budget)}</td>
                      <td className="px-3 py-2 text-right font-mono text-cyan-200">{fmt(c.bpr_actual)}</td>
                      <td className="px-3 py-2 text-right font-mono text-slate-400">{fmt(c.dashboard_budget)}</td>
                      <td className="px-3 py-2 text-right font-mono text-emerald-300">{fmt(c.dashboard_actual)}</td>
                      ...
                      <td className="px-3 py-2">{c.status==='unmatched' && canEditBudget && (
                        <button onClick={async()=>{
                          try {
                            await api.importBPRItems(vesselId,[{code:c.code, name:c.sub_category, approved_budget:c.bpr_budget, cost_group:c.cost_group}],budgetYear);
                            const updated = {...bprData};
                            const idx = updated.comparison.findIndex(x=>x.code===c.code && x.sub_category===c.sub_category);
                            if(idx>=0) updated.comparison[idx] = {...updated.comparison[idx], status:'match', matched:true, dashboard_budget:c.bpr_budget};
                            setBprData({...updated});
                            loadAll();
                          } catch(e){alert(e.message);}
                        }} className="px-2 py-0.5 rounded text-[10px] font-medium text-emerald-300 bg-emerald-900/30 border border-emerald-800/30 hover:bg-emerald-900/50 whitespace-nowrap">
                          + Add to Dash
                        </button>
                      )}</td>
                    </tr>
            ...
            <div className="px-3 py-3 text-xs text-right text-slate-600" style={{borderTop:'1px solid rgba(255,255,255,0.04)'}}>
              {bprData.comparison.filter(c=>bprFilter==='All'||c.status===bprFilter).length} items · {bprData.bpr_rows} BPR line items parsed
            </div>
          </div>
        </>}
